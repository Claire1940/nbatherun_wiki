...
export async function GET() {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.nbatherun.wiki'
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'NBA The Run'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(`> ${siteName} is a fan guide hub covering release news, gameplay systems, and community resources.`)
  lines.push('')
